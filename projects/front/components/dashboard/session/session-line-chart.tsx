...
const SessionLineChart: React.FC<LineChartProps> = ({ data, options }) => {
  ...
  const convertData = (targetData: TargetDataType): SourceDataType => {
    const labels: string[] = targetData.map(item => item.month);
    const datasets: SourceDataType['datasets'] = [];

    const datasetKeys = Object.keys(targetData[0]).filter(
      key => key !== 'month'
    );
    ...
    <Box className={`w-full h-full ${poppins.className}`}>
      <Box className="w-full h-full px-8 py-8 bg-white rounded-2xl flex-col">
        <Box className="justify-between w-full h-full font-semibold">
          ...
          <Box className="w-full h-full items-end flex-wrap justify-between">
            {sourceData.datasets.map((item, indx) => {
              return (
                <Box
                  className="items-center space-x-2 justify-start"
                  key={indx}
                >
